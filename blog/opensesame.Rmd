...
You will need to have OpenSesame installed: [https://osdoc.cogsci.nl](https://osdoc.cogsci.nl). There are also several tutorials on that page. The code used in the following tutorial is a mix of my own clumsy code and more sophisticated stuff borrowed from [this attentional-blink tutorial](https://osdoc.cogsci.nl/3.2/tutorials/advanced/) and [Bruno Nicenboim](). 
...
In the steps below, we're going to re-create the experiment structure above. Open a new OpenSesame experiment and click on *New experiment* in the Overview pane. This will open a tab in the main area called *General properties*. In *General properties*, I use `xpyriment` for the `Back-end`. On computers with limited graphics capabilities, you might have to choose `legacy`. This can be changed any time without breaking the experiment. The screen resolution should be that of your lab presentation PC. You can adjust the default font size/colour etc. to suit your lab.

<center>
![](images/general_tab.PNG){width=75% height=75%}
</center>

Next, right-click on the `sequence` ![](images/sequence_icon.PNG) called `experiment` in the experiment overview and give your experiment a name. Delete the two objects that are currently sitting under `experiment`.

**Very important:** Save your experiment, and keep saving it throughout.


### Step 1: Add a test mode

In later steps, we're going to send triggers via the parallel port. Read more about parallel ports in OpenSesame ![here](https://osdoc.cogsci.nl/3.2/manual/devices/parallel/). But if you're using a computer that doesn't have a parallel port (i.e. pretty much any computer that's not in your EEG lab), OpenSesame will throw an error if you try to send a trigger. We want to be able to test our paradigm without the port, so we can create a test mode where triggers are simply printed in OpenSesame's code console rather than sent to the port.

From the set of objects on the left, drag and drop an `inline_script` item ![](images/python_icon.PNG) onto the *experiment* sequence. It should appear one level below the sequence, as in the screenshot above. Right click on the inline script item and rename it *settings*. Here we will put some general settings and functions we need for the experiment, starting with our test mode. 

Python elements have two elements: a prepare phase and a run phase. You can read about the difference between these two phases ![here](https://osdoc.cogsci.nl/3.3/manual/python/about/).
...
Then paste the following on the `Run` tab to give you a warning that you're in test mode:
...
Next, we need to tell OpenSesame what order we want things to happen in for each practice trial (sentence), so drag and drop a sequence ![](images/sequence_icon.PNG) onto the loop. You will get a prompt; select `Insert into practice_loop`:

<center>
![](images/sequence_insert.png){width=50% height=50%}
</center>

Right click on your new sequence and rename it *practice_trial*. In the end, our practice trial should look something like this:
...
Translated, this code means: send trigger 194, wait a pre-specified interval of time, then send a zero trigger to reset the port. You need the time interval between the 194 and 0 triggers because otherwise they'll get sent at the same time and you'll only be able to see the zero in your EEG recording. The pre-specified interval of time should be added to the `Prepare` tab of the *settings* inline script:
...
Next, we add a jitter screen ![](images/jitter_icon.PNG) (call it *jitter*). This is just a blank screen that has a different, randomly selected duration for each sentence so that participants don't get entrained to the rhythm of the experiment. I set `Duration` to 1000 ms with a jitter of 100 ms and jitter mode standard deviation:

<center>
![](images/jitter_settings.PNG){width=50% height=50%}
</center>

Then, we add an inline script item to send another trigger:
...
Then we add another Python item which we'll call *RSVP*. This is where we'll program the sentences to be presented word-by-word, but we'll leave that until Step 5.

After RSVP, we insert another trigger that marks that the sentence is finished:
...
Then, after some sentences we ask participants questions. To present the question, we insert a sketchpad. Add some text to the sketchpad, here is an example:

<center>
![](images/sketchpad_question.PNG){width=50% height=50%}
</center>

OpenSesame knows that whatever is written between the square brackets is a variable that exists in our experimental stimuli. I'll go into detail about adding stimuli in Step 4, but in general, you would have a table that has columns such as *item*, *condition*, *sentence*, *question*, *correct_response*. OpenSesame will automatically pull out the contents of the column *question* that corresponds to the sentence just shown. Everything on the sketchpad that is not between square brackets will stay the same every time, e.g. here I have a prompt to remind participants which key corresponds Yes/No. Set the `Duration` of this sketchpad to zero: the response object coming up will take care of the sketchpad presentation duration.

Next we send a trigger to indicate when the question was asked:
...
Send another trigger to signal the response:
...
Then of course we send a trigger to indicate the break:
...
<center>
![](images/stimuli_from_file.PNG){width=50% height=50%}
</center>
...
<center>
![](images/stimuli_from_table.PNG){width=50% height=50%}
</center>
...
Make sure that the column names of your experimental table or file correspond exactly to the names of these variables that you call elsewhere in the experiment, e.g. on the question sketchpad or in Python code items.
...
<center>
![](images/stimuli.PNG){width=50% height=50%}
</center>
...
Note that I use a variable presentation duration for each word depending on the word's length, except for the target word, which I present for 700 ms. But you could also just set each word to appear for the same duration.

For my triggers, I base them on my experimental conditions, which I have numbered from 0 to 3 (note that Python starts counting at zero). So that I can later identify which triggers belong to which condition, I multiply the word number by the condition number. For the target word, I send a different (also condition-based) trigger: this makes it easier to extract target words from the EEG when you have targets in different sentence positions between conditions or items. The target word trigger is sent whenever the word number matches the column *target_wordno* in the stimuli table. Since Python starts counting words from zero, *target_wordno* should be 1 less than the actual word number.
...
Next, go to the *RSVP* Python item in *practice_trial*. In the `Prepare` tab of *RSVP*, we will use the `stim_prep` function we defined in *settings* to chop up the sentence for the current trial into words and compute a presentation duration and trigger number for each word. Paste in the following code:
...
Finally, you might like to show participants a message when they finish the practice phase and the experiment. Drag and drop sketchpad onto the *experiment* sequence, so that they're both at the same level as the *instructions* sketchpad. Drag them so that one is after the *practice_loop* and one after the *experimental_loop*. Edit the sketchpads to show participants a message like "That was the practice phase! If you are ready to continue, press the spacebar".
...
<center>
![](images/randomisation_table.PNG){width=50% height=50%}
</center>
...
<center>
![](images/stimuli_from_file_randomisation.PNG){width=50% height=50%}
</center>
...
```{r}
# split stimuli into lists
# *** This had to be done manually in R; the writing to file seemed to mess up the encoding and/or commas ***
# 'r' and 'w' in lines 39 and 46 were rt and wb in Bruno's script, which seems only
# compatible with Python 2. The below version is compatible with Python 3.
