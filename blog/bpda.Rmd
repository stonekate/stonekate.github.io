...
3. Resample the data 2000 times with replacement and repeat steps 1-2 after each resample.
 ...
 The procedure thus has 3 distinct components: 
...
#### Limitations of the bootstrap procedure
 ...
 As mentioned earlier, the bootstrap makes some unlikely assumptions ([Bååth, 2018](http://www.sumsar.net/blog/2015/04/the-non-parametric-bootstrap-as-a-bayesian-model/)):
...
Our prior distributions for the match and mismatch conditions therefore looked like this:
...
likelihood_sd_mismatch   <- sd(bootstrap_samples$mismatch, na.rm = TRUE)*
                                 (length(bootstrap_samples$mismatch)-1)/length(bootstrap_samples$mismatch)
...
match_lik <- ggplot(data.frame(x = c(0, 1200), cond = "Match"), aes(x = x)) + 
  ...
  scale_x_continuous(breaks = seq(0, 1200, 200), labels = seq(0, 1200, 200)) +
  labs(x = "Match onset (ms)", y = "Density") +
  theme_light() +
  theme(axis.text.y = element_blank(),
...
mismatch_lik <- ggplot(data.frame(x = c(0, 1200), cond = "Mismatch"), aes(x = x)) + 
  stat_density(data = bootstrap_samples, geom = "line",
...
\end{aligned}
$$
Via the same calculation, the posterior for the mismatch onset is 705 ms (SD = 9 ms): 
...
posterior_sd_mismatch    <- sqrt( (likelihood_sd_mismatch^2 * prior_sd^2) / (likelihood_sd_mismatch^2 + prior_sd^2) )
```


We can add these posteriors to our plots and see that they resemble the bootstrap data:

```{r, fig.height=3, fig.width=8, fig.align="center"}

match_post <- ggplot(data.frame(x = c(0, 1200), cond = "Match"), aes(x = x)) + 
  stat_density(data = bootstrap_samples, geom = "line",
               aes(match, colour = "bootstrap data", linetype = "bootstrap data"), 
               bw=10) +
  stat_density(data = bootstrap_samples, geom = "area",
...
mismatch_post <- ggplot(data.frame(x = c(0, 1200), cond = "Mismatch"), aes(x = x)) + 
  stat_density(data = bootstrap_samples, geom = "line",
               aes(mismatch, colour = "bootstrap data", linetype = "bootstrap data"), 
               bw=10) +
  stat_density(data = bootstrap_samples, geom = "area",
...
match_prior_inf <- ggplot(data.frame(x = c(0, 1200), cond = "Match"), aes(x = x)) + 
  stat_density(data = bootstrap_samples, geom = "line",
               aes(match, colour = "bootstrap data", linetype = "bootstrap data"), 
               bw=10) +
  stat_density(data = bootstrap_samples, geom = "area",
...
mismatch_prior_inf <- ggplot(data.frame(x = c(0, 1200), cond = "Mismatch"), aes(x = x)) + 
  stat_density(data = bootstrap_samples, geom = "line",
               aes(mismatch, colour = "bootstrap data", linetype = "bootstrap data"), 
               bw=10) +
  stat_density(data = bootstrap_samples, geom = "area",
...
posterior_mean_difference  <- posterior_mean_mismatch - posterior_mean_match
posterior_sd_difference    <- sqrt( posterior_sd_mismatch^2 + posterior_sd_match^2 )
...
prior_mean_difference     <- prior_mean_diff
prior_sd_difference       <- prior_sd_diff
posterior_mean_difference <- posterior_mean_exp2
posterior_sd_difference   <- posterior_sd_exp2
...
ggplot(data.frame(x = c(-750, 750), cond = "Difference"), aes(x = x)) + 
  ...
  stat_function(geom = "area", fun = dnorm, n = 2000, 
                args = list(mean = posterior_mean_difference, sd = posterior_sd_difference),
...
- $H_1$ is our posterior distribution, 
- and $D$ is the data:
...
Using our Bayesian divergence point analysis, we find strong evidence that prediction onsets were slower when there were two conflicting gender cues. The posterior estimate of the "mismatch effect" size was 336 ms, with a 95\% credible interval of 288--384 ms. But our existing bootstrap procedure already led us to the same conclusion, so what was the advantage of adding Bayesian principles? 
